            ...
            try:
                with active_client.responses.stream(**api_kwargs) as stream:
                    ...
                    if (
                        ...
                        and (final_output is None or (isinstance(final_output, list) and len(final_output) == 0))
                        and (not isinstance(final_output_text, str) or not final_output_text.strip())
                    ):
                        # ChatGPT-backed Codex can emit valid text deltas during the
                        # stream, then finalize with an empty/partial response
                        # object (for example output=[], output omitted, or
                        # output_text=""). Preserve the streamed text so the
                        # turn does not fail as malformed.
                        return SimpleNamespace(
                            id=getattr(final_response, "id", None),
                            status=getattr(final_response, "status", None),
                            model=getattr(final_response, "model", None),
                            output=[] if final_output is None else final_output,
    ...
    def _run_codex_create_stream_fallback(self, api_kwargs: dict, client: Any = None):
        ...
        try:
            for event in stream_or_response:
                ...
                if event_type and "output_text.delta" in event_type:
                ...
                if terminal_response is not None:
                    ...
                    terminal_output_text = getattr(terminal_response, "output_text", None)
                    if (
                        streamed_text_parts
                        and (terminal_output is None or (isinstance(terminal_output, list) and len(terminal_output) == 0))
                        and (not isinstance(terminal_output_text, str) or not terminal_output_text.strip())
                    ):
                        return SimpleNamespace(
                            id=getattr(terminal_response, "id", None),
