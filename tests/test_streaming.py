...
class TestCodexStreamCallbacks:
    ...
    def test_codex_preserves_streamed_text_when_final_response_is_empty(self):
        from run_agent import AIAgent
        ...
        mock_event_text = SimpleNamespace(
            type="response.output_text.delta",
            delta="Hello from streamed Codex!",
        )
        mock_event_done = SimpleNamespace(
            type="response.completed",
            delta="",
        ...
        assert deltas == ["Hello from streamed Codex!"]

    def test_codex_preserves_streamed_text_when_final_response_omits_output(self):
        from run_agent import AIAgent
        ...
        mock_event_text = SimpleNamespace(
            type="response.output_text.delta",
            delta="Missing output field finalization",
        )
        mock_event_done = SimpleNamespace(
            type="response.completed",
            delta="",
        ...
        assert response.output == []
        assert response.output_text == "Missing output field finalization"
    ...
    def test_codex_create_stream_fallback_preserves_streamed_text_when_terminal_response_is_empty(self):
        from run_agent import AIAgent
        ...
        response = agent._run_codex_create_stream_fallback(
            {"model": "gpt-5.3-codex", "instructions": "test", "input": []},
            client=mock_client,
        )
